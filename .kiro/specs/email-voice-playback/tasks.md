# Implementation Plan

## Tasks

### Phase 1: プロジェクト基盤構築

- [x] 1. Backend プロジェクトセットアップ
- [x] 1.1 FastAPI アプリケーションの初期化
  - Python 3.10+ のプロジェクト構成を作成する
  - FastAPI フレームワークと必要な依存関係をセットアップする
  - 非同期処理（async/await）の基本構造を準備する
  - 環境変数管理の仕組みを構築する
  - _Requirements: 1.1, 1.2_

- [x] 1.2 (P) データベーススキーマとモデル定義
  - PostgreSQL 用のテーブル定義を作成する（users, contacts, emails）
  - SQLAlchemy ORM モデルを実装する
  - マイグレーション管理の仕組みを構築する
  - OAuth トークン保存用のカラムを含める（gmail_refresh_token, gmail_access_token, gmail_token_expires_at）
  - _Requirements: 1.5, 6.5_

- [x] 1.3 (P) Frontend プロジェクトのVite移行
  - 既存のCreate React AppプロジェクトをViteに移行する
  - TypeScript設定を調整する
  - 開発サーバーとビルド設定を確認する
  - _Requirements: 4.1_

#### 📋 Phase 1 終了時のユーザー作業

> **開発者へ依頼**: 以下の作業を完了してください。
>
> **Google Cloud Console 作業**:
> - [x] Cloud SQL (PostgreSQL) インスタンスを作成する（Terraform または コンソール）
> - [x] データベースとユーザーを作成する
> - [x] 接続情報を環境変数に設定する
>
> **動作確認**:
> - [x] `npm run dev` でVite開発サーバーが起動することを確認
> - [x] Backend の FastAPI サーバーが起動することを確認

---

### Phase 2: 認証基盤

- [x] 2. Firebase 認証とGmail OAuth 連携
- [x] 2.1 Firebase Authentication ミドルウェア実装
  - Firebase Admin SDK を使用したトークン検証機能を実装する
  - Authorization ヘッダーからトークンを抽出する
  - 無効なトークンに対して 401 Unauthorized を返す
  - ユーザー情報（firebase_uid, email）をリクエストコンテキストに注入する
  - _Requirements: 6.3, 6.4, 6.5_

- [x] 2.2 Gmail OAuth サービス実装
  - OAuth 認証URL生成機能を実装する（state パラメータ含む）
  - 認証コードをトークンに交換する機能を実装する
  - Refresh Token / Access Token をデータベースに保存する
  - Access Token の有効期限切れ時に自動更新する機能を実装する
  - _Requirements: 1.3, 6.1, 6.2_

- [x] 2.3 Gmail OAuth API エンドポイント実装
  - GET /api/auth/gmail/url エンドポイントを実装する
  - POST /api/auth/gmail/callback エンドポイントを実装する
  - GET /api/auth/gmail/status エンドポイントを実装する
  - _Requirements: 1.3, 6.1, 6.2_

- [x] 2.4 (P) Frontend 認証コンテキスト実装
  - Firebase Authentication との連携を実装する
  - Google Sign-In フローを実装する
  - ID Token の取得・保持・自動更新機能を実装する
  - Gmail 連携状態の管理機能を実装する
  - Gmail 連携ボタンと OAuth フローの UI を実装する
  - _Requirements: 6.1, 6.2_

#### 📋 Phase 2 開始前のユーザー作業

> **開発者へ依頼**: Phase 2 のコード実装前に以下を完了してください。
>
> **Firebase Console 作業**:
> - [x] Firebase プロジェクトを作成する（GCP プロジェクトと紐付け）
> - [x] Authentication > Sign-in method で **Google** を有効化する
> - [x] プロジェクト設定 > サービスアカウント > 新しい秘密鍵を生成（JSON）
> - [x] Firebase SDK の設定情報（apiKey, authDomain 等）を取得する
>
> **Google Cloud Console 作業**:
> - [x] APIs & Services > OAuth consent screen を設定する（External, Testing）
> - [x] Test users に開発メンバーの Gmail アドレスを追加する
> - [x] APIs & Services > Credentials > OAuth 2.0 Client ID を作成する（Web application）
> - [x] 承認済みリダイレクト URI を設定する
> - [x] Client ID と Client Secret を環境変数に設定する
> - [x] Gmail API を有効化する

#### 📋 Phase 2 終了時のユーザー作業

> **動作確認**:
> - [x] Frontend で Google ログインができることを確認
> - [x] Gmail 連携ボタンをクリックし、OAuth 認証フローが動作することを確認
> - [x] 認証後に Backend で Refresh Token が保存されていることを確認

---

### Phase 3: メール受信・処理パイプライン

- [x] 3. Webhook とメール処理
- [x] 3.1 Pub/Sub Webhook ハンドラー実装
  - POST /api/webhook/gmail エンドポイントを実装する
  - Pub/Sub メッセージのデコード処理を実装する
  - 即座に 200 OK を返却し、BackgroundTasks で非同期処理を開始する
  - 重複通知の検出機能を実装する（historyId ベース）
  - _Requirements: 1.1, 1.2_

- [x] 3.2 Gmail API メール取得機能実装
  - Gmail API を使用してメール本文を取得する機能を実装する
  - 送信者情報（メールアドレス、名前）を抽出する
  - 登録済み連絡先かどうかを検証する機能を実装する
  - 未登録連絡先の場合は処理をスキップする
  - _Requirements: 1.3, 1.4_

- [x] 3.3 メール処理オーケストレーション実装
  - メール取得からギャル語変換、音声生成までの処理フローを実装する
  - 各処理段階でデータベースを更新する（is_processed フラグ管理）
  - エラー発生時のログ記録とリトライ処理を実装する
  - _Requirements: 1.5, 2.1, 3.1_

#### 📋 Phase 3 開始前のユーザー作業

> **開発者へ依頼**: Webhook 受信のために以下を完了してください。
>
> **Google Cloud Console 作業**:
> - [x] Cloud Pub/Sub API を有効化する
> - [x] Pub/Sub トピックを作成する（例: `gmail-notifications`）
> - [x] トピックに `gmail-api-push@system.gserviceaccount.com` の Publisher 権限を付与する
> - [x] Pub/Sub サブスクリプションを作成する（Push 型、Webhook URL 指定）
>
> **Cloud Run デプロイ**（Webhook 受信のため）:
> - [x] Backend を Cloud Run にデプロイする（Terraform または コンソール）
> - [x] Webhook URL を Pub/Sub サブスクリプションに設定する

#### 📋 Phase 3 終了時のユーザー作業

> **Gmail Watch 設定**:
> - [x] Gmail API の `users.watch` を実行して、Pub/Sub トピックへの通知を有効化する
>   ```bash
>   # ワンショットスクリプトまたは gcloud コマンドで実行
>   ```
>
> **動作確認**:
> - [x] テストメールを送信し、Webhook が受信されることを確認（ログ確認）
> - [x] メール本文が取得できることを確認

---

### Phase 4: AI サービス連携

- [x] 4. Gemini ギャル語変換サービス
- [x] 4.1 Gemini API クライアント実装
  - google-genai SDK を使用した API クライアントを実装する
  - ギャル語変換用のシステムプロンプトを定義する
  - 送信者名をプロンプトに埋め込む機能を実装する
  - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.5_

- [x] 4.2 ギャル語変換処理実装
  - メール本文を入力としてギャル語に変換する機能を実装する
  - 変換結果をデータベースに保存する（converted_body）
  - API エラー時のリトライ処理を実装する（指数バックオフ）
  - _Requirements: 2.6, 2.7_

- [x] 5. 音声合成サービス
- [x] 5.1 Cloud TTS API クライアント実装
  - Google Cloud TTS SDK を使用した音声合成機能を実装する
  - 日本語女性音声（ja-JP-Wavenet-B）を設定する
  - MP3 形式で音声データを生成する
  - _Requirements: 3.1, 3.2_

- [x] 5.2 GCS アップロード機能実装
  - 生成した音声ファイルを Cloud Storage にアップロードする機能を実装する
  - 公開 URL を生成する
  - ファイル名の命名規則を実装する（{email_id}_{timestamp}.mp3）
  - _Requirements: 3.3, 3.4_

- [x] 5.3 音声処理フロー統合
  - ギャル語変換完了後に音声合成を実行する処理フローを統合する
  - 音声 URL をデータベースに保存する（audio_url）
  - 全処理完了時に is_processed を true に更新する
  - TTS API エラー時のリトライ処理を実装する
  - _Requirements: 3.5, 3.6_

#### 📋 Phase 4 開始前のユーザー作業

> **開発者へ依頼**: AI サービス利用のために以下を完了してください。
>
> **Google Cloud Console 作業**:
> - [x] Vertex AI API または Generative Language API を有効化する
> - [x] Gemini API キーを取得し、環境変数に設定する
> - [x] Cloud Text-to-Speech API を有効化する
>
> **Terraform 作業** (Cloud Storage バケット):
> - [x] `infrastructures/terraform.tfvars` に `gemini_api_key` を設定する
> - [x] `terraform plan` で変更内容を確認する
> - [x] `terraform apply` でCloud Storageバケットを作成する
>   - バケット名: `your-gcp-project-id-togenuki-audio`
>   - 公開設定: `allUsers` に `Storage Object Viewer` 権限（Terraform管理）
>   - ※ 本番環境では署名付き URL を使用してください

#### 📋 Phase 4 終了時のユーザー作業

> **動作確認**:
> - [x] テストメールを送信し、ギャル語変換が実行されることを確認
> - [x] 音声ファイルが GCS にアップロードされることを確認
> - [x] 音声 URL にアクセスして再生できることを確認

---

### Phase 5: フロントエンド UI

- [x] 6. ダッシュボード機能
- [x] 6.1 メール一覧取得 API 実装
  - GET /api/emails エンドポイントを実装する
  - 認証済みユーザーの自分のメールのみ取得可能とする
  - 受信日時の降順でソートする
  - _Requirements: 4.1, 4.2, 4.3, 6.5_

- [x] 6.2 メール一覧コンポーネント実装
  - API からメール一覧を取得して表示する機能を実装する
  - カード形式で送信者名、件名、変換後テキストを表示する
  - 処理中のメール（is_processed=false）にはローディング表示を行う
  - _Requirements: 4.3, 4.4, 4.5_

- [x] 7. 音声再生機能
- [x] 7.1 音声プレイヤーコンポーネント実装
  - HTML5 Audio を使用した音声再生機能を実装する
  - 再生ボタンクリックで音声ファイルを読み込み再生する
  - 再生中は停止ボタンに切り替える
  - 停止ボタンクリックで音声再生を停止する
  - 再生完了時にボタン状態を元に戻す
  - _Requirements: 5.1, 5.2, 5.3, 5.4_

- [x] 7.2 音声再生エラーハンドリング実装
  - 音声ファイル読み込み失敗時のエラーメッセージ表示を実装する
  - ネットワークエラー等の例外処理を実装する
  - _Requirements: 5.5_

#### 📋 Phase 5 終了時のユーザー作業

> **動作確認**:
> - [x] ダッシュボードにメール一覧が表示されることを確認
> - [x] カード形式で送信者名、件名、変換後テキストが表示されることを確認
> - [x] 再生ボタンをクリックして音声が再生されることを確認
> - [x] 停止ボタンで音声が停止することを確認
>
> **デプロイ**:
> - [x] `npm run build` でフロントエンドがビルドされることを確認
> - [x] Cloud Run へのデプロイを実行

---

### Phase 6: 統合とテスト

- [x] 8. エンドツーエンド統合
- [x] 8.1 メール受信から音声再生までの統合テスト
  - Pub/Sub Webhook → メール取得 → ギャル語変換 → 音声生成 → DB保存のフロー確認
  - ダッシュボード表示 → 音声再生のフロー確認
  - 認証フロー（Firebase → Gmail OAuth）の確認
  - _Requirements: 1.1, 1.2, 1.3, 1.4, 1.5, 2.1, 2.6, 3.1, 3.3, 3.5, 4.1, 5.1, 6.1, 6.2, 6.3_

- [x] 8.2 ユニットテスト実装
  - GeminiService のテスト（プロンプト生成、エラーハンドリング）
  - TTSService のテスト（音声合成、GCS アップロード）
  - AuthMiddleware のテスト（トークン検証、エラーケース）
  - EmailProcessor のテスト（処理フロー、連絡先チェック）
  - _Requirements: 2.7, 3.6, 6.4_

#### 📋 Phase 6 終了時のユーザー作業（デモ準備）

> **エンドツーエンド動作確認**:
> - [x] 上司役アカウントから「至急！！」などのメールを送信する
> - [x] ダッシュボードでメールが表示されることを確認
> - [x] ギャル語に変換されたテキストを確認
> - [x] 再生ボタンで音声を聴き、ギャル口調で読み上げられることを確認
>
> **デモシナリオリハーサル**:
> - [x] ログイン → Gmail連携 → メール受信 → 音声再生 の一連のフローをリハーサル

---

## Requirements Coverage

| Requirement | Tasks |
|-------------|-------|
| 1.1 | 3.1, 8.1 |
| 1.2 | 1.1, 3.1, 8.1 |
| 1.3 | 2.2, 2.3, 3.2, 8.1 |
| 1.4 | 3.2, 8.1 |
| 1.5 | 1.2, 3.3, 8.1 |
| 2.1 | 3.3, 4.1, 8.1 |
| 2.2 | 4.1 |
| 2.3 | 4.1 |
| 2.4 | 4.1 |
| 2.5 | 4.1 |
| 2.6 | 4.2, 8.1 |
| 2.7 | 4.2, 8.2 |
| 3.1 | 3.3, 5.1, 8.1 |
| 3.2 | 5.1 |
| 3.3 | 5.2, 8.1 |
| 3.4 | 5.2 |
| 3.5 | 5.3, 8.1 |
| 3.6 | 5.3, 8.2 |
| 4.1 | 1.3, 6.1, 8.1 |
| 4.2 | 6.1 |
| 4.3 | 6.1, 6.2 |
| 4.4 | 6.2 |
| 4.5 | 6.2 |
| 5.1 | 7.1, 8.1 |
| 5.2 | 7.1 |
| 5.3 | 7.1 |
| 5.4 | 7.1 |
| 5.5 | 7.2 |
| 6.1 | 2.2, 2.3, 2.4, 8.1 |
| 6.2 | 2.2, 2.3, 2.4, 8.1 |
| 6.3 | 2.1, 8.1 |
| 6.4 | 2.1, 8.2 |
| 6.5 | 1.2, 2.1, 6.1 |
